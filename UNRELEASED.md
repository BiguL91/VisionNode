...
- **Overlay-Cache mit Smart-Invalidierung**: Overlay-Pixmap (Match-Boxen, OCR-Regionen, Scanned-Regions) wird nur dann neu gerendert, wenn sich die Overlay-Daten tatsächlich geändert haben (Vergleich per `!=`). Matching läuft ~12–15×/sec → Cache-Rebuild nur bei echten Änderungen, nicht 60×/sec.
